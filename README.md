...
Next, consider setting up required output folders and clearing out any old files (if you don't want to append to old files)
```
#THIS WILL MAKE THE OUTPUT AND MERGED FOLDERS IF THEY DON'T EXIST
uf.usefulFunctions().makedirs("Output/Merged/")

#IF YOU DON'T CLEAR OUT THE OUTPUT FOLDER THEN DATA FROM THIS RUN WILL BE
#APPENDED TO PRIOR DATA. IF YOU WANT TO APPEND THEN COMMENT OUT THIS LINE
uf.usefulFunctions().cleardir("Output/")
```
Then point to your participant folders
```
directories = glob.glob("Data/*/") #AN ARRAY OF ID FOLDERS
```

Finally, loop through your files and process/summarize
```

#LOOP THROUGH THE ID FOLDERS
for directory in directories:
    #FOR E/ ID, TAP INTO PHYSICAL ACTIVITY AND SLEEP FOLDER; WILL EXTRACT ALL STEP, HR, AND SLEEP .JSON FILES
    activity_dir = directory + "Physical Activity/"
    sleep_dir = directory + "Sleep/"

    #THE HR-BASED WEARTIME DETECTION PROCESS - PASS IN THE PA FOLDER AND WHERE WT DATA ARE TO BE STORED
    wt_obj = wt.wearTime(activity_dir,wt_output)

    #AFTER CORE PROCESSING, WT_OBJ WILL CONTAIN A .DAILY DATAFRAME WITH DAILY VALIDATION
    #IF NO HR DATA WERE AVAILABLE THEN THIS WILL BE EMPTY AND DON'T CONTINUE WITH NEXT STEPS
    if not wt_obj.saver.empty:
        #NOW PROCESS THE ACTIVITY DATA
        #PASS IN DAILY WEARTIME OBJECT, THE OUTPUT FOLDER FOR MIN-LEVEL STEPS, THE INPUT DIR, AND STARTDATE FILE (OPT)
        ap_obj = ap.activityProcess(wt_obj.daily,longform_steps, activity_dir, startdate_file)

        #IF WE WANT CONDENSED DATA WE CAN ALSO RUN .CONDENSE WITH AN OUTPUT DIRECTORY
        # (DAILY/WEEKLY FILES AUTOMATICALLY MADE) AND EITHER "D" FOR DAILY OR "W" FOR WEEKLY
        ap_obj.condense("Output/","W")

        #NEXT, PROCESS THE SLEEP DATA, PASSING THE FOLDER W/ SLEEP DATA, THE OUTPUT FILE, AND START DATE (OPTIONAL)
        sl_obj = sp.sleepProcess(sleep_dir,longform_sleep, startdate_file)

        #AGAIN, WE CAN CONDENSE TO SINGLE DAILY AND WEEKLY VALUES
        #ADD_NAPS IS OPTIONAL AND TRUE BY DEFAUL
        #SET TO FALSE IF YOU DON'T WANT NAPS TO COUNT TO TOTAL DAILY SLEEP
        sl_obj.condense('Output/','W',add_naps=True)

        #ONCE WE HAVE SLEEP DATA PROCESSED WE CAN PASS THE SLEEP OBJECT BACK TO THE ACTIVITY CLASS TO
        #MERGE AT THE DAILY AND/OR WEEKLY LEVEL
        ap_obj.addSleep('Output/Merged/',sl_obj,'W')
```

Key considerations
- the weartime processing must come first, as those data are used in subsequent steps
- activity and sleep can be processed independently
- the condense and merge steps are optional, though condensing should be done before the merge
  - if condense isn't run, then you will retain only the long-form files
    - Physical activity: 1440 minute-level daily steps
    - Sleep: One row per sleep/nap event
- if a startdate file is not provided then the first available data will be used to compute days and weeks since start and to do weekly summaries
- for sleep, you can include the option to add_naps (true by default) which will add nap times to total daily sleep. Set to false if you don't want this
```
  sl_obj.condense('Output/','W',add_naps=False)
```
